...
our %config = (
  ...
  openssldir => "",
  options => "-static --prefix=/etc/build_lib --cross-compile-prefix=aarch64-linux-gnu- no-afalgeng no-asan no-buildtest-c++ no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-pic no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-threads no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.26.1",
  perlargv => [ "linux-aarch64", "-static", "-no-shared", "--prefix=/etc/build_lib", "--cross-compile-prefix=aarch64-linux-gnu-" ],
  perlenv => {
      "AR" => undef,
      "BUILDFILE" => undef,
      "CC" => undef,
